...
class Generator {
  ...
  */
	getCost() {
		...
		return Number((this.baseCost * Math.pow(1 + _constants2.default.growthRatio, this.quantity)).toFixed(2));
	}
...
function loop(store) {
	...
	store.state.generators.forEach(generator => {
		...
		cookie += gen.rate * gen.quantity;
	});

	store.dispatch({
		type: _constants2.default.actions.add
	});
	...
	switch (action.type) {
		...
		case _constants2.default.actions.CHECK_STORY:
			...
			state.stories.forEach(story => {
				let sto = new _story2.default(story);
		...
		handleStateChange(newState) {
			...
			this.store.state.stories.forEach(story => {
				if (story.state === 'visible') {
					this.innerHTML = `<div>${story.name}<br>${story.description}</div>`;
				}
			});
		}
